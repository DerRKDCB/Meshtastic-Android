...
class PacketRepositoryImpl(private val dbManager: DatabaseProvider, private val dispatchers: CoroutineDispatchers) :
    ...
    override fun getImageChunksFrom(
        contact: String,
        recentChunkLimit: Int,
        getNode: suspend (String?) -> Node,
    ): Flow<List<Message>> =
        dbManager.currentDb
            .flatMapLatest { db ->
                db.packetDao().getRecentImageChunksFrom(contact, recentChunkLimit.coerceAtLeast(1))
                    .mapLatest { recentPackets ->
                        val dao = db.packetDao()
                        val packetsToRender =
                            dao.extendImageChunksForOldestPartialPayload(
                                contact = contact,
                                recentPackets = recentPackets,
                            )

                        packetsToRender
                            .sortedWith(compareBy<PacketEntity> { it.packet.received_time }.thenBy { it.packet.uuid })
                            .map { packet -> packet.toMessage(getNode) }
                    }
            }
